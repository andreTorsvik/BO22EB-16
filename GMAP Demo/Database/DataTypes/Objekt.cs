...
namespace GMAP_Demo
{
    public class Objekt
    {
        public int Id_ressurs { get; set; } // Løpenummer_ressurs som skriver til databasen må bruke DEFAULT pga constraintSequence
        ...
        public string ressursDataTypeToString // Her kan vi definere en versjon av string som viser datatypen i listbox
        {
            get
            {
                return $"({Id_ressurs} - {Navn} - {Kategori} - {Dato_opprettet} - {Opprettet_av_bruker} - {Sikkerhetsklarering} - {Kommentar} - {Lat} - {Lang})";
            }
        }
        ...
        public List<string> hentTags()
        {
            ...
            var TagListe = DBComTag_Ressurs.ListTag_ressursFromDb(Id_ressurs);
